...
class EnsembleService {
  ...
  private shouldAutoDisband(team: EnsembleTeam): boolean {
    ...
    // `ensemble steer` exists so a user can redirect a running team, so a sentinel or
    // sign-off from before that redirect no longer describes the work: an agent that
    // had already finished is not finished with the new instruction.
    //
    ...
    const isAfterLastRedirect = (message: EnsembleMessage): boolean => {
      if (!lastUserMessageMs) return true
